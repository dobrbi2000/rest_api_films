...
func (ac *ActorRepository) Create(a *models.Actor) (*models.Actor, error) {
	...
	if err := ac.storage.db.QueryRow(query, a.Name, a.Gender, a.BirthDate).Scan(&a.ID); err != nil {
		return nil, err
	}
...
func (ac *ActorRepository) SelectAll() ([]*models.Actor, error) {
	...
	for rows.Next() {
		a := models.Actor{}
		filmIDs := []int{}
		err := rows.Scan(&a.ID, &a.Name, &a.Gender, &a.BirthDate, pq.Array(&filmIDs))
		if err != nil {
			log.Println(err)
			continue
		}
		if actor, ok := actorsMap[a.ID]; ok {
